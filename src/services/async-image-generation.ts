 ...
 */
export async function checkGenerationStatus(id: string): Promise<GenerationStatus | null> {
 ...
 */
export async function getLatestImage(downloadPath?: string): Promise<string> {
	const script = `
 ...
 */
async function pollChatGPTUI(): Promise<{isGenerating: boolean, hasRecentImage: boolean}> {
	const script = `
		tell application "ChatGPT"
			activate
			delay 0.5
 ...
 */
export function cleanupGenerations(): void {
	tracker.cleanup();
}
